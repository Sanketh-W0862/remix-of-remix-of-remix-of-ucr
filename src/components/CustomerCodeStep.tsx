...
const CustomerCodeStep = ({ onNext, onBack }: CustomerCodeStepProps) => {
  ...
  const handleContinue = () => {
    ...
    onNext({ hasCode, existingCode, formData, uploadedDocs });
  };
            ...
            <div className="glass-card p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
               {[
                  ...
                  <button
                    key={doc.key}
                    onClick={() => { toggleDoc(doc.key); setShowDocError(false); }}
                  ...
                  >
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{uploadedDocs[doc.key] ? "Uploaded" : "Click to upload"}</p>
                  </button>
                ))}
              </div>
              {showDocError && (
